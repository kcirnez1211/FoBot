...
class OandaApi:
    ...
    def get_account_ep(self,ep,data_key): #ep = endpoint
    ...
    def get_candles_df(self,pair_name,**kwargs):
        ...
        for candles in data:
            if candles['complete']==False:
                continue
            new_dict = {}
            new_dict['time'] = parser.parse(candles['time'])
            new_dict['volume'] = candles['volume']
            for p in prices:
                if p in candles:
                    for o in ohlc:
                        new_dict[f"{p}_{o}"] = float(candles[p][o])
    ...
    def close_trade(self,trade_id):
        ...
        if ok==True:
            ...
            print(f"Error closing trade {trade_id}") 
