...
from models.trade_decision import TradeDecison
...
def place_trade(trade_decision:TradeDecison,api:OandaApi,log_message,log_error,trade_risk):
    ...
    if trade_id is None:
        log_error(f"Failed to place trade:{trade_decision}")
        log_message(f"Failed to place trade:{trade_decision}",trade_decision.pair)
    else:
        log_message(f"Trade placed:{trade_id} for {trade_decision}",trade_decision.pair)