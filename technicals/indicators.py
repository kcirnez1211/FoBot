...
#most of the code are formulas in this page.Formula Calculation
...
def BollingerBands(df:pd.DataFrame,n=20,s=2):
    ...
    df['BB_UP'] = df['BB_MA'] + stddev * s #formula based for bollinger
    df['BB_LW'] = df['BB_MA'] - stddev * s
    return df

def ATR(df:pd.DataFrame,n=14): #Average true range
...
def KeltnerChannels(df: pd.DataFrame,n_ema = 20,n_atr=10):
    df['EMA'] = df.mid_c.ewm(span=n_ema,min_periods=n_ema).mean() #exponential moving average
    df = ATR(df,n=n_atr)
    c_atr = f"ATR_{n_atr}"
...
def RSI(df:pd.DataFrame,n=14):
    ...
    losses = pd.Series([x * -1 if x<0 else 0.0 for x in gains],name="loss")
